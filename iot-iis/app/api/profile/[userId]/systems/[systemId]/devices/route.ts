...
export const POST = async (request: NextRequest, { params }) => {
    ...
    try {
        ...
        const addedDevice = await prisma.device.update({
          ...
          data: {
            systemId: params.systemId, //tady se nejak musi dostat to ID
          },
        });
...
export const DELETE = async (request: NextRequest, { params }) => {
    ...
    try {
        ...
        return new Response(`Successfuly deleted device ${deletedDevice.deviceId}`, {status: 200});
    }
    catch (err) {
        return new Response("Could not delete device from system", {status: 500});
    }
}